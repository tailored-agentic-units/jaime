...
Jekyll 4.x blog deployed to GitHub Pages via GitHub Actions. Hosted at `{{URL}}{{BASEURL}}/`.

## Key Details

- **baseurl**: `{{BASEURL}}` — all asset references must use `| relative_url` filter
...
- **Syntax highlighting**: Rouge (dark + light) via `prefers-color-scheme`
...
bundle exec jekyll serve           # local preview at localhost:4000{{BASEURL}}/
```
